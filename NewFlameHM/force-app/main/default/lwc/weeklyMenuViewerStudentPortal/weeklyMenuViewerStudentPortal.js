...
import getMessSchedule from '@salesforce/apex/MessMenuControllerForStudent.getMessSchedule';
import getDailyMenuItemsStudentPortal from '@salesforce/apex/MessMenuControllerForStudent.getDailyMenuItemsStudentPortal';
...
export default class WeeklyMenuViewerStudentPortal extends LightningElement {
    @api messId;
    @api selectedDate;
    selectedMenuId;
    startDate;
    ...
    connectedCallback() {
        ...
        if (this.messId && this.startDate && this.endDate) {
            this.loadData();
        }
    }
    ...
    loadData() {
        ...
        this.fetchMenuSchedule()
            .then(() => {
                ...
                return this.fetchDailyMenuItems();
            })
            .then((dailyItems) => {
                this.noRecords = dailyItems.length == 0?true:false;
    ...
    fetchMenuSchedule() {
        return getMessSchedule({ messId: this.messId })
            .then(result => {
    ...
    fetchDailyMenuItems(refreshToken) {
        ...
        return getDailyMenuItemsStudentPortal(params);
    ...
    removeEmptyRows() {
        ...
        const rows = this.template.querySelectorAll('.menu-item-row');

        rows.forEach(row => {
            const cells = Array.from(row.querySelectorAll('td')); // Get all cells in the row
            const categoryCell = cells[0].textContent.trim(); // Get the first cell's value

            // Check if the first cell (category) is not empty and all other cells contain only "-"
            const allOtherCellsHaveDash = cells.slice(1).every(cell => cell.textContent.trim() === '-');

            // Remove the row if all conditions are met
            if (categoryCell && allOtherCellsHaveDash) {
                row.remove();
            }
        });
    ...
    forceRefresh() {
        ...
        getDailyMenuItemsStudentPortal({
            messId: this.selectedMessId,
            startDate: this.startDate,
            endDate: this.endDate,
